...
export function normalize(source, raw, checkedAt) {
  ...
  return {id:raw.id?.startsWith('import-') ? raw.id : 'import-'+source.id+'-'+createHash('sha256').update(apply).digest('hex').slice(0,16),
    ...
    checkedAt, ts:0, status:'live', imported:true};
}
export function parseFeed(source, data, checkedAt) {
  ...
  if(source.kind==='ashby') {
    ...
    rows=data.jobs.map(j=>({role:j.title,apply:j.jobUrl,isListed:j.isListed,type:typeName(j.employmentType),
    ...
    rows=data.jobs.map(j=>({role:j.title,apply:j.absolute_url,deadline:j.application_deadline,location:j.location?.name,
