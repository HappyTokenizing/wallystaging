...
Listings show the employer's posting date and default to the last 30 days, with 7, 60 and 90-day alternatives. Results are ordered newest first. Imported dates come from Ashby `publishedAt`, Greenhouse `first_published`, or the employer listing's structured `JobPosting.datePosted`; update and scan dates are never substituted. Undated roles remain in company coverage but are excluded from recency-filtered results. Locally submitted roles use their submission timestamp.

The featured member carousel has previous/next arrows and always excludes roles older than 30 days, even when the listing filter is wider. It selects newest roles first while limiting any company to two consecutive cards, including the wraparound. Excess same-company roles remain in the main list. A single-company spotlight stays static. Automatic rotation pauses during hover/focus and is disabled for reduced-motion preferences.

`GET /api/job-feed` reads the allowlisted public Ashby, Greenhouse, Lever (US/EU), Rippling, Teamtailor RSS and Traffit sources in `data/job-sources.json`. Responses are cached for 30 minutes. A successful empty feed removes that source’s roles; a failed feed uses the dated committed snapshot for at most seven days. No new paid service, credentials or cron are required. `node scripts/refresh-jobs.mjs` refreshes the deployment fallback.
...
The September 26 employer expansion adds the requested non-member employers. See [verified source mapping](data/JOB-SOURCES.md) for official careers URLs, feed/date provenance and employers with no recent public vacancies. The member roster is unchanged.
